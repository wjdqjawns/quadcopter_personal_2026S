...
Trajectory::Trajectory(std::vector<Waypoint> waypoints)
    : wps_(std::move(waypoints))
{
    if (wps_.size() < 2)
        throw std::invalid_argument("Trajectory needs at least 2 waypoints");
}

double Trajectory::duration() const { return wps_.back().arrival_time; }

bool Trajectory::isComplete(double t) const { return t >= duration(); }
...
Trajectory Trajectory::hover(const Vec3& pos, double dur) {
    return Trajectory({{pos, Vec3::Zero(), 0.0},
                       {pos, Vec3::Zero(), dur}});
}

Trajectory Trajectory::circle(const Vec3& center, double radius,
                               double height, double period, double dur) {
    const int N = 48;
